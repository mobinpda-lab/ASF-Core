...
def classify_provider_pressure(
    *,
    ...
    """Classify provider pressure into a bounded control-plane action."""
    text = message.lower()
    ...
    if status_code in {502, 503, 504} or any(
        token in text
        for token in ("temporarily unavailable", "upstream unavailable", "service unavailable")
    ):
    ...
    if status_code == 422 or any(
        token in text for token in ("routing rejected", "invalid model route", "unsupported route")
    ):
        return ProviderPressureDecision(
...
def pressure_allows_retry(
    ...
    """Allow only bounded retries for explicitly retryable provider pressure."""
    return decision.retryable and max_attempts > 0 and attempt < max_attempts
