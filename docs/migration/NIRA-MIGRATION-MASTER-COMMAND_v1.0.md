...
## Mission
Build NIRA as an independent autonomous software factory platform while preserving proven knowledge, evidence, documentation, history, and lessons from the client repositories. Migrate factory authority to NIRA and connect products through adapters without duplicating authorities or stopping product development.
...
## Execution phases
1. Complete discovery across the registered client repositories.
...
3. Build the independent NIRA platform from the existing factory foundation rather than duplicating it: registry, intake, queue, state machine, lease manager, worker identity, fencing, capacity control, provider abstraction, evidence ledger, promotion authority, audit, monitoring, recovery, memory, and learning.
...
## Operating rule
NIRA is the canonical forward implementation and operating identity. Historical provenance remains preserved in Git history and immutable provider records; it does not override the current NIRA naming contract.
