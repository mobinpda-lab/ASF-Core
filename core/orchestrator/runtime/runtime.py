...
class FailureAction(str, Enum):
    RETRY="RETRY"; WAIT="WAIT"; RECOVER="RECOVER"; BLOCK="BLOCK"
...
class DependencyEngine:
    ...
    def validate_graph(self)->None:
        ...
        def visit(task_id:str)->None:
            if task_id in visiting: raise ValueError(f"invalid dependency graph at {task_id}")
...
class RecoveryPolicy:
    _ACTIONS={FailureClass.TRANSIENT:FailureAction.RETRY,FailureClass.EVIDENCE:FailureAction.WAIT,FailureClass.VALIDATION:FailureAction.RECOVER,FailureClass.GOVERNANCE:FailureAction.BLOCK,FailureClass.UNKNOWN:FailureAction.BLOCK}
    def action(self,failure:FailureClass)->FailureAction: return self._ACTIONS[failure]

class OrchestratorRuntime:
    def __init__(self,tasks:Iterable[Task]=()):
        self.tasks={task.task_id:task for task in tasks}; self.contexts={}; self.evidence={}; self.history=[]
    ...
    def _create_context(self,task:Task)->ExecutionContext:
        lifecycle=Lifecycle(task.state); context=ExecutionContext(task.task_id,f"ctx-{task.task_id}",self._timestamp(),lifecycle); self.contexts[task.task_id]=context; return context
    def intake(self,task:Task)->ExecutionContext:
        ...
        self.tasks[task.task_id]=task; self.dependencies=DependencyEngine(self.tasks); self.dependencies.validate_graph(); return self._create_context(task)
    ...
    def decide(self,task_id:str,failure:Optional[FailureClass]=None)->DecisionRecord:
        ...
        context=self.contexts[task_id]; previous=context.lifecycle.state; decision,reason,target=Decision.BLOCK,"",previous
        if failure is not None:
            action=self.recovery.action(failure); reason=f"failure={failure.value}; action={action.value}"
            decision={FailureAction.RETRY:Decision.WAIT,FailureAction.WAIT:Decision.WAIT,FailureAction.RECOVER:Decision.RECOVER,FailureAction.BLOCK:Decision.BLOCK}[action]
            ...
            else: decision,reason=Decision.ALLOW,"dependencies, evidence, and state valid"; target=LifecycleState.QUEUED if previous==LifecycleState.CREATED else previous
        if target!=previous: context.lifecycle.transition(target,reason)
        record=DecisionRecord(decision,reason,self._timestamp(),previous,target,task_id); self.history.append(record); return record
