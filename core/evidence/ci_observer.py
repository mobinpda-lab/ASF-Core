...
class EvidenceObserver:
    def observe(self, repository: str, commit_sha: str, source: Mapping[str, Any]) -> EvidenceRecord:
        ...
        critical=("workflow_runs","check_runs","jobs")
        if any(name not in source for name in critical):
        ...
        if provider_state is ObservationState.INCONSISTENT: state=Visibility.NOT_EXPOSED
        elif provider_state is ObservationState.DELAYED: state=Visibility.PENDING
    ...
    @staticmethod
    def _record(repository,commit_sha,values,state,provider,confidence,reason):
    ...
    def _validate_sha(sha):
        if len(sha)!=40 or any(c not in "0123456789abcdef" for c in sha.lower()): raise ValueError("commit_sha must be a 40-character hexadecimal SHA")
    @staticmethod
    def _reject_mismatch(repository,commit_sha,groups:Iterable[Iterable[Mapping[str,Any]]]):
        for group in groups:
            for item in group:
                if item.get("repository") not in (None,repository): raise ValueError("evidence repository mismatch")
                observed=item.get("commit_sha") or item.get("head_sha")
                if observed not in (None,commit_sha): raise ValueError("stale or mismatched commit SHA")
    @staticmethod
    def _classify(values):
        runs,checks,jobs=values["workflow_runs"],values["check_runs"],values["jobs"]
        if not runs and not checks and not jobs and not values["statuses"]: return Visibility.NOT_FOUND
        conclusions=[str(x.get("conclusion","")).lower() for x in (*runs,*checks,*jobs)]
        if any(c in {"failure","cancelled","timed_out","action_required"} for c in conclusions): return Visibility.FAILURE
        if any(c in {"","queued","in_progress","pending","waiting"} for c in conclusions): return Visibility.PENDING
