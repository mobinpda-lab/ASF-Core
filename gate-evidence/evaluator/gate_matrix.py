...
def evaluate_matrix(repository:str,commit_sha:str,required_gates:Iterable,expected_branch:str=None)->PromotionResult:
 ...
 if expected_branch is not None and any(getattr(g,'branch',None) not in (None,expected_branch) for g in gates): return PromotionResult(repository,commit_sha,PromotionDecision.BLOCK,'wrong branch')
 d=evaluate(gates)
